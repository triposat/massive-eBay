...
class LoggingConfig:
    ...
    @staticmethod
    def setup(log_file: str = "ebay_scraper.log") -> logging.Logger:
...
class ProductExtractor:
    """Methods for extracting product information from HTML"""
    ...
    @staticmethod
    def extract_title(soup: BeautifulSoup) -> tuple[Optional[str], Optional[str]]:
        title = subtitle = None
    ...
    @staticmethod
    def extract_price(soup: BeautifulSoup) -> tuple[Optional[str], Optional[str], Optional[str]]:
        current_price = was_price = discount = None
        if price_section := soup.select_one("div.x-price-section"):
            if current_price_elem := price_section.select_one("div.x-price-primary span.ux-textspans"):
                current_price = current_price_elem.text.strip()
            if transparency_section := price_section.select_one("div.x-price-transparency"):
                if discount_container := transparency_section.select_one("span.x-price-transparency--discount"):
                    if was_price_elem := discount_container.select_one("span.ux-textspans--STRIKETHROUGH"):
                        was_price = was_price_elem.text.strip()
                    if discount_span := discount_container.select_one("span.ux-textspans--EMPHASIS"):
                        discount = discount_span.text.strip()
        return current_price, was_price, discount

    @staticmethod
    def extract_shipping(soup: BeautifulSoup) -> tuple[Optional[str], Optional[str]]:
            ...
            if shipping_row := shipping_div.select_one(
                ...
                if shipping_section := shipping_row.select_one("div.ux-labels-values__values-content"):
                    if first_div := shipping_section.find('div'):
                        for span in first_div.find_all("span", class_="ux-textspans", recursive=False):
                            if (
                                span.text.strip()
                                ...
                                and not span.get('class', [''])[0] == 'ux-textspans__custom-view'
                            ):
                    ...
                    if location_div := shipping_section.select_one("div:nth-child(2)"):
                        if location_span := location_div.select_one("span.ux-textspans--SECONDARY"):
                            ...
                            if "Located in:" in location_text:
                                ...
                                location = location_text.split("Ubicado en:")[-1].strip()
                            else:
                                location = location_text
        return shipping, location

    @staticmethod
    def extract_availability(soup: BeautifulSoup) -> tuple[Optional[str], Optional[str]]:
    ...
    @staticmethod
    def extract_returns(soup: BeautifulSoup) -> Optional[str]:
    ...
    @staticmethod
    def extract_condition_brand_type(soup: BeautifulSoup) -> tuple[Optional[str], Optional[str], Optional[str]]:
        condition = brand = type_ = None
        ...
        if (brand_dl := soup.select_one("dl.ux-labels-values--brand")) and (
            ...
            brand_value := brand_div.select_one(".ux-labels-values__values .ux-textspans")
        ):
            brand = brand_value.text.strip()
        ...
        return condition, brand, type_


class EbayRealTimeScraper:
    """Main scraper class for eBay product listings"""
    ...
    def __init__(
        ...
        self._init_domain_settings(domain)

        # Initialize proxy settings
        self._init_proxy_settings()

        # Initialize scraper configuration
        self._init_scraper_config(
    ...
    def _init_domain_settings(self, domain: str):
    ...
    def _init_scraper_config(self, max_retries: int, max_concurrent_requests: int, batch_size: int, max_items: Optional[int]):
        """Initialize scraper configuration"""
    ...
    def validate_page_content(self, html_content: str, page_type: str) -> Tuple[bool, str]:
        """Validate the HTML content of a page"""
                ...
                async with self.semaphore:
                    ...
                    if response.status_code == 200:
                        ...
                        if is_valid:
                            ...
                            logger.warning(f"Attempt {retries + 1}: Invalid content: {message}")
                        ...
                        logger.warning(f"Attempt {retries + 1}: Got status {response.status_code} for {url}")
                        await asyncio.sleep(self.retry_delay * (retries + 1))
                        retries += 1
                        continue
                    else:
    ...
    async def process_search_page(self, session: AsyncSession, page_num: int, search_term: str) -> Tuple[bool, bool]:
        ...
        while retry_count < max_page_retries:
            try:
                url = self._build_search_url(search_term, page_num)
                logger.info(f"Processing search page {page_num} (Attempt {retry_count + 1})")
                ...
                return await self._process_page_content(session, html_content, search_term)
    ...
    def _build_search_url(self, search_term: str, page_num: int) -> str:
        ...
        return self.base_search_url + urlencode(params)

    async def _process_page_content(self, session: AsyncSession, html_content: str, search_term: str) -> Tuple[bool, bool]:
        """Process the content of a search page"""
        soup = BeautifulSoup(html_content, "lxml")
        product_urls = self.extract_product_urls(soup)

        if not product_urls:
            logger.info(f"No more products found for '{search_term}' after {self.items_scraped} items")
            return False, True
    ...
    async def _process_product_urls(self, session: AsyncSession, product_urls: List[str]):
        ...
        for i in range(0, len(product_urls), chunk_size):
            ...
            chunk = self._get_product_chunk(product_urls[i:i + chunk_size])
            results = await self._scrape_product_chunk(session, chunk)
    ...
    def _get_product_chunk(self, chunk: List[str]) -> List[str]:
        """Get appropriate sized chunk of products based on remaining items"""
        if self.max_items:
            remaining_items = self.max_items - self.items_scraped
            return chunk[:remaining_items]
        return chunk

    async def _scrape_product_chunk(self, session: AsyncSession, chunk: List[str]) -> List[Dict]:
        """Scrape a chunk of products"""
    ...
    async def _handle_results(self, results: List[Dict]):
        ...
        self.items_scraped += len(results)
        self.products.extend(results)
        await self.force_save_buffer()
        logger.info(f"Progress: {self.items_scraped}/{self.max_items if self.max_items else 'unlimited'} items")
    ...
    async def scrape_product(self, session: AsyncSession, url: str) -> Optional[Dict]:
        ...
        try:
            ...
            # Extract store info
            details.store_info = ProductExtractor.extract_store_info(soup)

            # Extract product details using ProductExtractor methods
            details.title, details.subtitle = ProductExtractor.extract_title(soup)
            details.current_price, details.was_price, details.discount = ProductExtractor.extract_price(soup)
            details.shipping, details.location = ProductExtractor.extract_shipping(soup)
            details.availability, details.sold_count = ProductExtractor.extract_availability(soup)
            details.returns = ProductExtractor.extract_returns(soup)
            details.condition, details.brand, details.type = ProductExtractor.extract_condition_brand_type(soup)
    ...
    async def _update_product_data(self, existing_data: Dict, batch: List[Dict]) -> Dict:
        """Update product data with new batch"""
        new_products = [product for product in batch if product["url"] not in self.saved_product_ids]
    ...
    def _update_save_statistics(self, batch_size: int):
        """Update save statistics"""
        self.total_saved = len(self.saved_product_ids)
        logger.info(f"Saved batch of {batch_size} new products. Total saved: {self.total_saved}")

    async def _create_emergency_backup(self, batch: List[Dict]):
        """Create emergency backup in case of save failure"""
        try:
            backup_file = f"{self.current_output_file}.emergency_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            emergency_data = {
                "products": batch,
            ...
            logger.info(f"Created emergency backup: {backup_file}")
...
def get_search_terms() -> List[str]:
    ...
    return [term.strip() for term in input("\nEnter search terms (comma-separated): ").strip().split(",")]


def get_max_items() -> Optional[int]:
    """Get maximum items from user input"""
    max_items_input = input("\nEnter maximum items to scrape (leave empty for all): ").strip()
    return int(max_items_input) if max_items_input else None
